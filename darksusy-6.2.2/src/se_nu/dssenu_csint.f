...
***********************************************************************
      ...
      real*8 v,res,dssenu_csint2,r,max,dssem_sunvesc,
     &  umin,umax,foveru,sigsd,vp
      integer vtype
      external foveru
      common/seint/mx,ma,sigsi,sigsd,rx,vtype
      external dssenu_csint2
      include 'dssecom.h'
      include 'dsmpconst.h'
...
c...the condition umax above.
      ...
      if (umin.lt.umax) then
        call dssenu_hiprecint2(dssenu_csint2,foveru,umin,umax,res)
        res=res*1.0d5    ! km/s -> cm/s from u integration
        dssenu_csint=res*r*r*4.*pi  ! assume spherical sun
      else
        dssenu_csint=0.0d0
      endif
