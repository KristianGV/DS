...
c----------------------------------------------------------------------
       ...
       real*8 m(11),f(11),phi(11),mn,sla1,fact,sig_a,dssenu_sefull
       integer i
       mn=(m_p+m_n)/2.0d0
       m(1)=16.d0*mn
       m(2)=28.d0*mn
...
c...the Earth. We use Eq. (A10) in gould. dssenu_sefull returns everything in
...
c...number of atoms of the given type.
     ...
     &   dssenu_sefull(mx,m(i),v_star,v_bar,phi(i))*(rho_x/mx)*v_bar
     &          *fact
c         write(*,*) 'caprate: ',sla1
c      capture rate on earth per second
 20    continue
        dssenu_capearthfull=sla1
        return
        end
