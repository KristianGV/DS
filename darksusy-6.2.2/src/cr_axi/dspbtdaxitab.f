...
ccc
      ...
      real*8 tp,result,dspbtdaxi
      real*8 xmax,xmin,xint,ymax,ytoll,yfrac
      character*200 pbfile,scr
...
ccc
      ...
      integer isstore,powerstore,dmdihalotagpb,nocraxirz,nopbcraxi,iload
      character*10 dmslabelt
      common/pbtdaxitabcom/xtd,ytd,ytd2,realntd,isstore,powerstore,
     &  dmdihalotagpb,nocraxirz,nopbcraxi,iload,dmslabelt
ccc
      dhow=how
      if(dhow.lt.1.or.dhow.gt.4) then
        write(*,*) 'DS: ERROR in dspbtdaxitab: the requested how = ',
...
ccc      
        ...
        parvec(6)=dble(isin)
        call dspbtdaxidb(dmdihalotag,parvec,dmdtmp,check,match)
        if(match) then
          dspbtdaxitab=parvec(1)
          return
        endif
ccc
ccc link directly to dspbtdaxi
ccc
c        write(*,*) 'computing pb : ',dmdihalotag,tpin
        dspbtdaxitab=dspbtdaxi(R,z,tpin,powerin,isin,pbprec,pbnprec
     &   ,labhalo,loadlabloc)
ccc
ccc store the value of dspbtdaxi:
ccc      
        check=.false.
        parvec(1)=dspbtdaxitab
        call dspbtdaxidb(dmdihalotag,parvec,dmdtmp,check,match)
        return
      endif
ccc
...
ccc
      if(dmdihalotagpb.ne.dmdihalotag) then
        dmslabelt=dmdlabel
        dmdihalotagpb=dmdihalotag
        iload=0
      endif  
ccc
ccc check the match with the propagation parameters. NOTE: this works
ccc only if you properly set the integer dspbcraxino via dspbcraxick      
ccc
...
ccc generate file name and store variables
        call dsdatafile(pbfile,'pbtdaxitab-')
        call dscharadd(pbfile,rzaxitag)
        call dscharadd(pbfile,'-')
        call dscharadd(pbfile,pbcraxitag)
        call dscharadd(pbfile,powerlab)
        call dscharadd(pbfile,islab)
        call dscharadd(pbfile,'-')
        call dscharadd(pbfile,dmslabelt)
...
ccc
        write(*,*) 'DS: starting dspbtdaxi tabulation'  
        ymax=0.d0
        xmin=0.1d0
        xmax=10000.d0
        ii=0
...
ccc
        do i=0,100
          ...
          result=dspbtdaxi(R,z,tp,powerin,isin,pbprec,pbnprec,labhalo
     &      ,loadlabloc)
          ii=ii+1
          xtd(ii)=dlog(tp)
          ytd(ii)=result
...
ccc    
     ...
     &      ,loadlabloc)
            ...
            if(npoints.le.1000) then
              ...
              write(*,*) 'DS: in dspbtdaxitab exceeded the maximum'
        ...
        enddo
        continue
        write(*,*) 'DS: dspbtdaxi tabulation is over'  
      elseif(dhow.eq.4.and.iload.ne.1) then  ! read table from file
        if (prtlevel.ge.2) write(*,*) 'DS: Reading pb table from file '
     &    ,pbfile
        ii=1 
        open(unit=13,file=pbfile,status='old',form='formatted',
     &       err=200)
...
ccc
      if (dhow.eq.3) then ! write table to disk
        write(*,*) 'DS: Writing pb table to file ',pbfile
        open(unit=13,file=pbfile,status='unknown',form='formatted')
...
ccc now do the actual table lookup
     ...
     &  .or.tpin.lt.dexp(xtd(1))) then
        write(*,*) 'DS: dspbtdaxitab called for tp = ',tpin
        write(*,*) 'DS: out of the tabulated range :',dexp(xtd(1)),
     &    dexp(xtd(int(realntd)))
        write(*,*) 'DS: dspbtdaxitab set to zero '
        dspbtdaxitab=0.d0
      else   
...
ccc
 201  close(13)
      write(*,*) 'DS: link to a pbtdaxi table file ',pbfile
...
ccc
 200  close(13)
      write(*,*) 'DS: The requested pbtdaxi table file ',pbfile
...
ccc
      ...
      subroutine dspbtdaxidb(dmdindex,parvec,tmp,check,match)
ccc
...
ccc      
     ...
     &  parextralc(npartot) 
      common/pbtdaxidbcom/tmpdatj,datj,parextralc,dmdihatmpdatj,
