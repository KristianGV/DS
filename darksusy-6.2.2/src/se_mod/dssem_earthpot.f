...
**********************************************************************
      ...
      real*8 radius(42),pot(42),ppl,r,dssem_earthmass,
     &  dssem_earthpotint
      integer i,j
     ...
     &  6375140.0d0,6378140.0d0/
      ...
      if (first) then
        do i=1,42
          pot(i)=dssem_earthpotint(radius(i))
        enddo
        first=.false.
      endif

      if (r.ge.6378.140d3) then
        dssem_earthpot=-dssem_earthmass(6378.140d3)*1.d-3*GNewton/r
        return
      endif

      if (r.le.0.0d0) then
        dssem_earthpot=pot(1)
        return
      endif

      call dshunt(radius,42,r,j)

      ppl=(r-radius(j))/(radius(j+1)-radius(j))

      dssem_earthpot=pot(j)*(1.0d0-ppl)+pot(j+1)*ppl
