...
***********************************************************************
      ...
      implicit none
      include 'dsmpconst.h'
      real*8 r,m_a,dssem_earthdens,rcore,fraction
      integer a,index
...
c...give the same total mass fractions. With these values, dssenu_capearthfull
c...and dssenu_capearthnumi (with the same gaussian) are expeted to give
...
c     &           0.0d0   ,0.0d0/      ! Cr
      ...
      elseif (a.eq.52) then ! Cr
        ...
        write(*,*) 'warning in dssem_earthdenscomp:'
        write(*,*) '  element ',a,' requested - not available!'
        write(*,*) '  please update dssem_earthdenscomp'
        dssem_earthdenscomp=0.0d0
        return
      endif
...
c...m_a is now in atomic units u which is what we want
      ...
      dssem_earthdenscomp=fraction*dssem_earthdens(r)/m_a*n_avogadro ! number/cm^3
