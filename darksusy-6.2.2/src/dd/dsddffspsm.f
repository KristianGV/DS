      subroutine dsddffspsm(q,a,z,l2jjpp,l2jjnn,l2jjpn,ierr)
...
c=======================================================================
      ...
      integer n,i,dsnucldindx,l
      real*8 y,j,ff,r,rsprc
...
! this section tries to find l using the energy levels of a harmonic oscillator potential with an example of spin-orbit coupling like in Wong (1999)
        l=-1
        if (n.le.2.and.j.eq.0.5d0) then
          l=0
        else if (n.le.8.and.(j.eq.0.5d0.or.j.eq.1.5d0)) then
          l=1
        else if (n.le.20) then
          if (j.eq.0.5d0) then
            l=0
          else if (j.eq.1.5d0.or.j.eq.2.5d0) then
            l=2
          endif
        else if (n.le.50) then
          if (j.eq.3.5d0.or.j.eq.2.5d0) then
            l=3
          else if (j.eq.1.5d0.or.j.eq.0.5d0) then
            l=1
          else if (j.eq.4.5d0) then
            l=4
          endif
        else if (n.le.82) then
          if (j.eq.3.5d0.or.j.eq.4.5d0) then
            l=4
          else if (j.eq.2.5d0.or.j.eq.1.5d0) then
            l=2
          else if (j.eq.0.5d0) then
            l=0
          else if (j.eq.5.5d0) then
            l=5
          endif
        else if (n.le.126) then
          if (j.eq.4.5d0) then
            l=5
          else if (j.eq.3.5d0.or.j.eq.2.5d0) then
            l=3
          else if (j.eq.6.5d0) then
            l=6
          else if (j.eq.1.5d0.or.j.eq.0.5d0) then
            l=1
          endif
        ...
        if (l.eq.-1) goto 1000
        l2jjpp=0.d0
        l2jjnn=j*(j+1.d0)/(2.d0*l+1.d0)**2
        l2jjpn=0.d0

      else                      ! odd proton
! this section tries to find l using the energy levels of a harmonic oscillator potential with an example of spin-orbit coupling like in Wong (1999)
        l=-1
        if (z.le.2.and.j.eq.0.5d0) then
          l=0
        else if (z.le.8.and.(j.eq.0.5d0.or.j.eq.1.5d0)) then
          l=1
...
c     &           'dsddspsmff: SPSM gaussian form factor unavailable ',
c     &           'for A=',a,' Z=',z
c            l2jjpp=-1.d0
c            l2jjnn=-1.d0
c            l2jjpn=-1.d0
...
c            return
          if (prtlevel.gt.0) then
            write (*,*)
     &           'dsddspsmff: SPSM spin to charge radius',
     &           ' relation not found ',
     &           'for A=',a,' Z=',z
            write (*,*) 
 ...
 1000 continue
     ...
     &     'dsddspsmff: SPSM form factor unavailable for A=',
     &     a,' Z=',z
      l2jjpp=-1.d0
      l2jjnn=-1.d0
      l2jjpn=-1.d0
