...
c-----------------------------------------------------------------------
      ...
      if (fi.lt.9.or.fi.gt.13) then ! all yields but dbar
         ...
         dsanyieldgetdiff=phidiff(zi,mxi,ch,fi)
         ...
         if (dpi.ge.-1) then
           dsanyieldgetdiff=(1.0-dpl)*phidiffdb(zi,dpi,mxi,ch,fi,kind)
     &       +dpl*phidiffdb(zi,dpi+1,mxi,ch,fi,kind)
         else
           dsanyieldgetdiff=0.0d0
         endif
      endif
