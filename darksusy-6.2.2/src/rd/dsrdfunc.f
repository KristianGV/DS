...
c-----------------------------------------------------------------------
      ...
      if (p.lt.1.0d-50) p=1.0d-50  ! tb necessary to avoid NaN in output!
...
c...both low and high masses
      ...
      if (dsisnan(dsrdfunc)) then
        if (prtlevel.ge.1) write(*,*) 'WARNING: dsrdfunc returned NaN !!!'
        if (prtlevel.ge.2) write(*,*) 'u,x,p,wrate(p) = ',u,x,p,wrate(p)
        dsrdfunc=1.0d-50
      endif
