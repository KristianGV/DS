      ...
      real*8 e_mu,rd,dssea_nuism
      integer fltype
      common/lbe_int3/e_mu,rd,fltype
      e_nu=x
      fluxnu=0.5d0*dssea_nuism(e_nu)
...
c      epsilon=522. ! rock values, je phd thesis
      ...
      if (fltype.eq.1) then ! flux of muons
        dssea_fff2=avogadro*sigma/alpha*(fluxnu+fluxnubar)
c      dssea_fff2=dssea_fff2/(1.+e_mu/epsilon)*
c     1      (e_nu-e_mu+a/3.*(e_nu-e_mu*(e_mu/e_nu)**2)/e_nu**2)
        dssea_fff2=dssea_fff2/(1.+e_mu/epsilon)*
     1        (e_nu-e_mu+a/3.*(e_nu-e_mu*(e_mu/e_nu)**2)) !je corr 980603
        dssea_fff2=dssea_fff2*1.d-12 ! 23 from avo; -38 from sigma; 3 from alpha
      else ! contained events
        dssea_fff2=avogadro*sigma*(fluxnu+fluxnubar)
        dssea_fff2=dssea_fff2*(1.0+a*(e_mu/e_nu)**2)
        dssea_fff2=dssea_fff2*1.d-15  ! 23 from avo; -38 from sigma
      endif
      return
      end
