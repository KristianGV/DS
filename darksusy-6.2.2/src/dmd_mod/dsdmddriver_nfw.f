      subroutine dsdmddriver_nfw(iwin,nrein,rein,nchin,chin,labin,reout)
...
ccc      
      ...
      real*8 r,rs,rhos,x,dsdmdprof_nfw,radouttr,radintr,normx,rho0,
     &  dsI1_nfw,units
...
c
        ...
        do ii=ndmdstart,ndmdlabtot
          ...
          enddo
          if(.not.match) then
            write(*,*) 'DS: in dsdmddriver_nfw missing initialization'
        ...
        if(mwlab) then
c
c adding rhosnfw, possibly in terms of the local halo density
c
          setrho=.false.
          do jj=1,nchin
            if(chin(jj).eq.chrho0) then      
              r=dmdparvec(ihalotag(iobjdist,ihalocur))
              rs=dmdparvec(ihalotag(irsnfw,ihalocur))
              x=r/rs
              normx=dsdmdprof_nfw(x)
              normx=rein(jj)/normx
              call dsdmdsetind(indexout,normx)
              ihalotag(irhosnfw,ihalocur)=indexout
              call dsdmdsetind(indexout,rein(jj))
          ...
          enddo
          do jj=1,nchin
            if(chin(jj).eq.chrhosnfw) then
              if(setrho) then
                write(*,*) 'DS: inconsistency !!! In dsdmddriver_nfw'
              ...
              r=dmdparvec(ihalotag(iobjdist,ihalocur))
              rs=dmdparvec(ihalotag(irsnfw,ihalocur))
              rhos=dmdparvec(ihalotag(irhosnfw,ihalocur))
              x=r/rs
              rho0=rhos*dsdmdprof_nfw(x)
          ...
          enddo
          if(.not.setrho) then
            write(*,*) 'DS: Error: In dsdmddriver_nfw you are'
...
c
        ...
        rhos=dmdparvec(ihalotag(irhosnfw,ihalocur))
        x=r/rs
        reout=rhos*dsdmdprof_nfw(x)  ! GeV cm^-3
        return
      endif
ccc
...
c
        ...
        rhos=dmdparvec(ihalotag(irhosnfw,ihalocur))
        x=(min(r,radouttr))/rs
        reout=rhos*4.d0*pi*rs**3*dsI1_nfw(x) ! GeV cm^-3 kpc^3
...
ccc      
      write(*,*) 'DS: linking to dsdmddriver_nfw with iwin = ',iwin
