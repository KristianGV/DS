...
!
!    subroutine input_map_s            (4/8)
!    subroutine read_bintab_s          (4/8)
!    subroutine read_conbintab_s           NotYet
!    subroutine write_bintab_s         (4/8)
!    subroutine write_asctab_s          NA
!    subroutine dump_alms_s                NotYet
!    subroutine write_alms_s               NotYet
!    subroutine read_alms_s                NotYet
!    subroutine read_bintod_s           (8)
!    subroutine write_bintabh_s         (8)
!    subroutine unfold_weights_s
!    subroutine unfold_weightslist_s
!
!
...
!=======================================================================
#ifndef NO64BITS
subroutine input_map4_s(filename, map, npixtot, nmaps, &
    ...
    npixtot8 = npixtot
    call input_map8_s(filename, map, npixtot8, nmaps, &
         fmissval, header, units, extno, ignore_polcconv)

    return
  end subroutine input_map4_s
#endif

!=======================================================================
subroutine input_map8_s(filename, map, npixtot, nmaps, &
     fmissval, header, units, extno, ignore_polcconv)
    !=======================================================================
...
9000 format(a)
  ...
  !=======================================================================
#ifndef NO64BITS
  subroutine read_bintab4_s(filename, map, npixtot, nmaps, nullval, anynull, header, units, extno)
    character(len=*),                          intent(IN)  :: filename
    integer(I4B),                              intent(IN)  :: npixtot
    integer(I4B),                              intent(IN)  :: nmaps
    ...
    call read_bintab8_s(filename, map, npixtot8, nmaps, nullval, anynull, header, units, extno)
    return

  end subroutine read_bintab4_s
#endif
  subroutine read_bintab8_s(filename, map, npixtot, nmaps, nullval, anynull, header, units, extno)
  !=======================================================================
    character(len=*),                          intent(IN)  :: filename
    integer(I8B),                              intent(IN)  :: npixtot
    integer(I4B),                              intent(IN)  :: nmaps
...
111    continue
       ...
       if (status == 202) then ! bad_data not found
          if (SP == SP) nullval = s_bad_value ! default value
          if (SP == DP) nullval = d_bad_value ! default value
          status = 0
       endif
    ...
    return
  end subroutine read_bintab8_s

  !=======================================================================
  subroutine read_conbintab_s(filename, alms, nalms, units, extno)
    !=======================================================================
    ...
    REAL(SP), DIMENSION(0:nalms-1,1:6), INTENT(OUT) :: alms
    CHARACTER(LEN=*), dimension(1:), optional,  INTENT(OUT) :: units
    INTEGER(I4B)                   , optional,  INTENT(IN) :: extno
    ...
    !        finds the bad data value
!     if (SP == SP) call ftgkye(unit,'BAD_DATA',nullval,comment,status)
!     if (SP == DP) call ftgkyd(unit,'BAD_DATA',nullval,comment,status)
    call f90ftgky_(unit, 'BAD_DATA', nullval, comment, status)
    if (status == 202) then ! bad_data not found
       if (SP == SP) nullval = s_bad_value ! default value
       if (SP == DP) nullval = d_bad_value ! default value
    ...
    do frow = 1, nrows, nrow2read
       ...
       do i = i0, i1
          ...
          alms(i,1) = real( l, kind=SP)
          alms(i,2) = real( m, kind=SP)
       enddo
       i0 = i1 + 1_i8b
    enddo
    deallocate(lm)
...
!     enddo
    ...
    if (status > 0) call printerror(status)
    return
  end subroutine read_conbintab_s

#ifndef NO64BITS
  subroutine write_bintab4_s(map, npix, nmap, header, nlheader, filename, extno)
  !=======================================================================
    INTEGER(I4B),     INTENT(IN) :: npix, nmap, nlheader
    REAL(SP),       INTENT(IN), DIMENSION(0:npix-1,1:nmap) :: map
    ...
    if (present(extno)) then
       call write_bintab8_s(map, npix8, nmap, header, nlheader, filename, extno)
    else
       call write_bintab8_s(map, npix8, nmap, header, nlheader, filename)
    endif
    return
  end subroutine write_bintab4_s
#endif

  subroutine write_bintab8_s(map, npix, nmap, header, nlheader, filename, extno)
    !=======================================================================
...
!     REAL(SP),       INTENT(IN), DIMENSION(0:npix-1,1:nmap), target :: map
    REAL(SP),       INTENT(IN), DIMENSION(0:npix-1,1:nmap) :: map
    CHARACTER(LEN=*), INTENT(IN), DIMENSION(1:nlheader)      :: header
    CHARACTER(LEN=*), INTENT(IN)           :: filename
    INTEGER(I4B)    , INTENT(IN), optional :: extno
...
!    character(len=80)         :: junk_k, junk_v, junk_c
    ...
    if (SP == SP) comment = 'data format of field: 4-byte REAL'
    if (SP == DP) comment = 'data format of field: 8-byte REAL'
    do i=1,nlheader
       card = header(i)
    ...
    return
  end subroutine write_bintab8_s
  !=======================================================================
  subroutine write_asctab_s &
    ...
    REAL(SP),        INTENT(IN)           ::  clout(0:lmax,1:ncl)
  ...
  !
  subroutine dump_alms_s(filename, alms, nlmax, header, nlheader, extno)
    !=======================================================================
    INTEGER(I4B),      INTENT(IN) :: nlmax, nlheader, extno
    COMPLEX(SPC),      INTENT(IN), DIMENSION(0:,0:) :: alms
    CHARACTER(LEN=80), INTENT(IN), DIMENSION(1:nlheader) :: header
    CHARACTER(LEN=*),  INTENT(IN)               :: filename
    ! local variables
    INTEGER(I4B),   DIMENSION(:),   allocatable :: lm
    REAL(SP),     DIMENSION(:,:), allocatable :: alms_out
    INTEGER(I4B) ::  status,unit,blocksize,bitpix,naxis,naxes(1)
...
!!    npix=((nlmax+1)*(nlmax+2))/2
    ...
    return
  end subroutine dump_alms_s
  !=======================================================================
  subroutine write_alms_s(filename, nalms, alms, ncl, header, nlheader, extno)
    !=======================================================================
    ...
    REAL(SP),        INTENT(IN), DIMENSION(0:nalms-1,1:(ncl+1)) :: alms
    CHARACTER(LEN=80), INTENT(IN), DIMENSION(1:nlheader) :: header
    CHARACTER(LEN=*),  INTENT(IN)               :: filename
...
!!!    stride = 1000 ! 1000 rows at a time
    ...
    return
  end subroutine write_alms_s
  !=======================================================================
  subroutine read_alms_s(filename, nalms, alms, ncl, header, nlheader, extno) 
    !=======================================================================
    !     Read a FITS file
    !
    !     slightly modified to deal with vector column 
    ...
    REAL(SP), DIMENSION(0:nalms-1,1:(ncl+1)), INTENT(OUT) :: alms
...
!        if (SP == DP) CALL ftgkyd(unit,'BAD_DATA',nullval,comment,status)
  ...
  !======================================================================================
  SUBROUTINE write_bintabh_s(tod, npix, ntod, header, nlheader, filename, extno, firstpix, repeat)
    !======================================================================================
    ...
    real(SP), dimension(:), allocatable :: padding
    integer(I8B) :: lastpix
    integer(I4B) :: lengap
...
!                   & ') will be used.'
    ...
    if (lengap > 0) then
       ...
       if (SP == SP) padding(:) = HPX_SBADVAL
       if (SP == DP) padding(:) = HPX_DBADVAL
       do colnum = 1, ntod
  ...
  END SUBROUTINE write_bintabh_s
  ! ==============================================================================
...
! 2018-05-25
!---------------------------------------
  subroutine unfold_weightsfile_s(w8file, w8map)

    USE pix_tools,      only: nside2npix, nside2npweights

    character(len=*), intent(in)                  :: w8file
    real(SP),       intent(out), dimension(0:)  :: w8map
    real(SP),       allocatable, dimension(:,:) :: w8list
    ...
    return
  end subroutine unfold_weightsfile_s
!---------------------------------------
...
! 2018-05-18
!---------------------------------------
  subroutine unfold_weightslist_s(nside, won, w8list, w8map)

    USE pix_tools,      only: nside2npix, nside2npweights
    USE long_intrinsic, only: long_size

    integer(i4b),            intent(in)  :: nside, won
    real(SP), dimension(0:), intent(in)  :: w8list
    real(SP), dimension(0:), intent(out) :: w8map
    ...
    return
  end subroutine unfold_weightslist_s
