...
!... DS mod (TB: messages only printed in debug mode, 2018-01-19)
 if(BRmodel.and.BRSM) then
  delta_rate%BRcov_ok=.True.
  if (debug) write(*,*) "Covariance matrix for relative branching ratio uncertainties read in successfully."
 else
 ...
 if(XSmodel.and.XSSM) then
  ...
  if (debug) write(*,*) "Covariance matrix for relative cross section uncertainties read in successfully."
 else
...
!------------------------------------------------------------
subroutine setup_model_rate_uncertainties(filename_XS, filename_BR)
!------------------------------------------------------------
 use usefulbits, only : file_id_common3
 use store_pathname_hs, only : pathname_HS
 use usefulbits_hs, only : delta_rate
 ...
 character(LEN=*),intent(in) :: filename_XS, filename_BR
 logical :: BRmodel, XSmodel
!... DS mod
    logical debug

 call read_matrix_from_file(9,filename_BR,delta_rate%BRcov, BRmodel)
 call read_matrix_from_file(5,filename_XS,delta_rate%CScov, XSmodel)

 if(BRmodel.and.XSmodel) then
  delta_rate%BRcov_ok=.True.
  delta_rate%CScov_ok=.True.
!... DS mod (TB: messages only printed in debug mode, 2018-01-19)
  if (debug) write(*,*) "Covariance matrices for rate uncertainties read in successfully."
 else
...
!... DS mod
 ...
 if(mode.eq.1.or.mode.eq.2) then
  mc_mode = mode
!... DS mod (TB: messages only printed in debug mode, 2018-01-19)
  if (debug) write(*,'(1X,A,A)') 'The mass-centered chi^2 method will treat the Higgs',&
...
!... DS mod (TB: messages only printed in debug mode, 2018-01-19)
   if (debug) write(*,'(1X,A,A,1I1,A)') 'Use a '//trim(pdf_desc(pdf))//' probability density function ',&
