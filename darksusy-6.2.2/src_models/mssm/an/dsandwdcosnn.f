...
c  date: 96-02-21
c  modified: 01-09-12
c  modifed: 15-10-29 added QCD NLO corrections (torsten bringmann)
c=======================================================================
      implicit none
      include 'dsmssm.h'
      include 'dsio.h'
     ...
     & sumf,imres,reres,gluons,gammas,zgam,qqv0(20:25,42:45),qqgv0(20:25,42:45)
      ...
      real*8 dsib3svqqratio, dsib3svqqgratio, dsansumaa

      integer dsidnumber
      integer idold(42:45) ! one for each neutralino pair
      data idold/-123456789,-123456789,-123456789,-123456789/
      save idold
c-----------------------------------------------------------------------
...
c... save zero-velocity result for quarks (added by TB)
c... FIXME: QCD corrections can in principle easily be extended to include the full
c... velocity dependence of the s-wave, by adapting the below dsanXXX
c... (or modify dsansumaa!?) to sum only over relevant helicites
c... disadvantage: needs to be computed for every p anew  
      if (idold(kp1).ne.dsidnumber().and.NLOoption.eq.'default'
     &    .and.kp1.eq.kp2) then
        do f=7,12
          call dsanclearaa
          do i=1,6
            if (f.eq.7.or.f.eq.9.or.f.eq.11) then    ! u, c, t
              ksf=ksqu(i)
            else                                     ! d, s, b
              ksf=ksqd(i)
            endif
            call dsantffsff(0.0d0,0.0d0,kp1,kp2,ksf,f,f)
            call dsanuffsff(0.0d0,0.0d0,kp1,kp2,ksf,f,f)
          enddo
          call dsansffsff(0.0d0,0.0d0,kp1,kp2,kh3,f,f)
          call dsansffvff(0.0d0,0.0d0,kp1,kp2,kz,f,f)
          qqv0(13+f,kp1)=ncolor(f)*(kk/sqrt(s))*dsansumaa()/(32.d0*pi)
        enddo
      endif
...
c...add qcd corrections to quark final states

        if (NLOoption.eq.'default'.and.kp1.eq.kp2.and.f.ge.7) then
            
           prtial(13+f)=prtial(13+f)-qqv0(13+f,kp1) ! subtract "s-wave part" at tree-level
                                                    ! (see comment on v-dependence above)
           prtial(13+f)=prtial(13+f)+
     &                  qqv0(13+f,kp1)*dsib3svqqratio(sqrt(s),kp1,f) ! add qcd-corrected version
        endif

c f fbar
      enddo

      if (kp1.eq.kn(1).and.kp2.eq.kn(1)) then   ! FIXME: in principle, the other
                                                ! neutralinos also contribute, in 
...
c... modified: 97-05-14 by j. edsjo to use new w-convention
        if (incglue) then
          if (idold(kp1).ne.dsidnumber()) then
...
c... added by tb
      ...
      if (kp1.eq.42) then  ! FIXME: IB routines currently cannot handle
                           ! other neutralinos
        if (NLOoption.eq.'default'.and.kp1.eq.kp2) then
          do f=7,12
            if (idold(kp1).ne.dsidnumber()) then
              qqgv0(13+f,kp1)=qqv0(13+f,kp1)*dsib3svqqgratio(kp1,f)              
            endif  
            prtial(27) = prtial(27) + qqgv0(13+f,kp1)
          enddo  
        endif
      endif
...
c===================================================== 
c... set flags for processes that should only be calculated once
    
      if (kp1.eq.kp2.and.idold(kp1).ne.dsidnumber()) then
        idold(kp1)=dsidnumber()
        if (kp1.eq.42) calc=1
      endif
