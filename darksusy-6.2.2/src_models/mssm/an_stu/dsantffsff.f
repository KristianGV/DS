...
****************************************************
      ...
      if (gl(kpk,kp3,kp1).eq.dcmplx(0.d0,0.d0).and.
     &    gr(kpk,kp3,kp1).eq.dcmplx(0.0d0,0.0d0).and.
     &    gl(kpk,kp2,kp4).eq.dcmplx(0.0d0,0.0d0).and.
     &    gr(kpk,kp2,kp4).eq.dcmplx(0.0d0,0.0d0)) return

      dt=1.0d0/dcmplx(mk**2-t,-width(kpk)*mk)
      gp2=gl(kpk,kp3,kp1)*gl(kpk,kp2,kp4)+
     &    gr(kpk,kp3,kp1)*gr(kpk,kp2,kp4)
      gm2=gl(kpk,kp3,kp1)*gl(kpk,kp2,kp4)-
     &    gr(kpk,kp3,kp1)*gr(kpk,kp2,kp4)
      gx2=gl(kpk,kp3,kp1)*gr(kpk,kp2,kp4)+
     &    gr(kpk,kp3,kp1)*gl(kpk,kp2,kp4)
      gy2=gl(kpk,kp3,kp1)*gr(kpk,kp2,kp4)-
     &    gr(kpk,kp3,kp1)*gl(kpk,kp2,kp4)

      aa(0,0,0,0)=aa(0,0,0,0)-(((gp2*kpl*ppl+gx2*kmi*pmi)*wd(1,0,0)-(
     . efmi*emi*gx2)-(efpl*epl*gp2))*dt)
      aa(0,0,1,-1)=aa(0,0,1,-1)-((efmi*gm2*ppl-(efpl*gy2*pmi)+gp2*kmi*
     . ppl+gx2*kpl*pmi)*wd(1,0,-1)*dt)
      aa(0,0,1,0)=aa(0,0,1,0)+((efmi*gy2*pmi-(efpl*gm2*ppl))*wd(1,0,0)
     . -(emi*gy2*kmi)+epl*gm2*kpl)*dt
      aa(0,0,1,1)=aa(0,0,1,1)-((efmi*gm2*ppl-(efpl*gy2*pmi)-(gp2*kmi*
     . ppl)-(gx2*kpl*pmi))*wd(1,0,1)*dt)
      aa(1,-1,0,0)=aa(1,-1,0,0)+(emi*gm2*kpl-(epl*gy2*kmi)-(gp2*kpl*
     . pmi)-(gx2*kmi*ppl))*wd(1,-1,0)*dt
