...
c   from DarkSUSY
     ...
     &              /dble(conjg(squrmx(4,1)))) +pi/2.0d0
      ...
      elseif(famu.eq.2) then
        if (mass(ksqu_flav(2,1)).ge.mass(ksqu_flav(2,2))) then  ! DarkSUSY convention matches
          w(1)=mass(ksqu_flav(2,1))**2/mass(kgluin)**2
          w(2)=mass(ksqu_flav(2,2))**2/mass(kgluin)**2
         thetasu=atan(-dble(conjg(squlmx(5,2)))
     &              /dble(conjg(squrmx(5,2)))) 
        else! DarkSUSY convention differs. Switch 1 <-> 2
          w(1)=mass(ksqu_flav(2,2))**2/mass(kgluin)**2
          w(2)=mass(ksqu_flav(2,1))**2/mass(kgluin)**2
         thetasu=atan(-dble(conjg(squlmx(5,2)))
     &              /dble(conjg(squrmx(5,2)))) +pi/2.0d0
      ...
      elseif(famu.eq.3) then
        ...
        if (mass(ksqu_flav(3,1)).ge.mass(ksqu_flav(3,2))) then   ! DarkSUSY convention matches
          thetasu=atan(-dble(conjg(squlmx(6,3)))
     &             /dble(conjg(squrmx(6,3))))
          w(1)=mass(ksqu_flav(3,1))**2/mass(kgluin)**2
          w(2)=mass(ksqu_flav(3,2))**2/mass(kgluin)**2

        else        ! DarkSUSY convention differs. Switch 1 <-> 2
          thetasu=atan(-dble(conjg(squlmx(6,3)))
     &             /dble(conjg(squrmx(6,3))))+pi/2.0d0
          w(1)=mass(ksqu_flav(3,2))**2/mass(kgluin)**2
          w(2)=mass(ksqu_flav(3,1))**2/mass(kgluin)**2
        endif
      ...
      elseif(famd.eq.2) then
        ...
        if (mass(ksqd_flav(2,1)).ge.mass(ksqd_flav(2,2))) then  ! conventions agree
          x1=mass(ksqd_flav(2,1))**2/mass(kgluin)**2
        else   ! conventions differ, switch 1 <-> 2
          x1=mass(ksqd_flav(2,2))**2/mass(kgluin)**2
        endif

      elseif(famd.eq.3) then

        if(mass(ksqd_flav(3,1)).ge.mass(ksqd_flav(3,2))) then  ! conventions agree
          x1=mass(ksqd_flav(3,1))**2/mass(kgluin)**2
        else  ! conventions differ, switch 1 <-> 2
          x1=mass(ksqd_flav(3,2))**2/mass(kgluin)**2
        endif
