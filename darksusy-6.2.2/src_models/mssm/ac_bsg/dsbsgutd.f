...
c   from DarkSUSY
     ...
     &             /dble(conjg(squrmx(6,3))))
        u1=mass(ksqu_flav(3,1))**2/mass(kgluin)**2
        u2=mass(ksqu_flav(3,2))**2/mass(kgluin)**2

      else        ! DarkSUSY convention differs. Switch 1 <-> 2
        thetast=atan(-dble(conjg(squlmx(6,3)))
     &             /dble(conjg(squrmx(6,3))))+pi/2.0d0
        u1=mass(ksqu_flav(3,2))**2/mass(kgluin)**2
        u2=mass(ksqu_flav(3,1))**2/mass(kgluin)**2
      endif
...
c     as well as a trilinear parameter
      ...
      elseif(famd.eq.3) then
       ...
       write(*,*) 'dsbsgutd called with wrong family nb.'  
       stop 
      endif 
...
c     from the top sector
      ...
      dsbsgutd=-0.5d0*(dsbsgh1x(x1)-(cos(thetast))**2*dsbsgh1x(u1)
