...
c     than the third one
      ...
      if (mass(ksqd_flav(3,1)).ge.mass(ksqd_flav(3,2))) then ! DS convention agrees
        x1=mass(ksqd_flav(3,1))**2/mass(kgluin)**2
        x2=mass(ksqd_flav(3,2))**2/mass(kgluin)**2    
      else ! conventions differ, switch 1 <-> 2
        x1=mass(ksqd_flav(3,2))**2/mass(kgluin)**2
        x2=mass(ksqd_flav(3,1))**2/mass(kgluin)**2    
      endif

      ad=asoftd(3)


      dsbsghd=0.5d0*(dsbsgh1x(x1)-(cos(thetast))**2*dsbsgh1x(u1)
