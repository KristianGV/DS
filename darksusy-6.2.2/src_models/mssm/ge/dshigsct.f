...
c=======================================================================
     ...
     &     mst2(2),msb2(2),delta0,ht2,hb2,au,ad,
...
c      real*8 mhch,halpha, mqbl,mqbr,mqtl,mqtr
      integer i,j,k,l,ierr,unphys,hwarning
...
c---------------- one-loop higgs sector (brignole-ellis-ridolfi-zwirner)
        ...
        if (mass(ksqu_flav(3,1)).eq.0.0)  call dssfesct(unphys)
        ...
        mt4 = mass(kt)**4
        mb4 = mass(kb)**4
        msqst1 = mass(ksqu_flav(3,1))**2
        msqst2 = mass(ksqu_flav(3,2))**2
        msqsb1 = mass(ksqd_flav(3,1))**2
        msqsb2 = mass(ksqd_flav(3,2))**2
...
c-------- one-loop higgs sector (drees-nojiri, prd45(92)2482 + my errata)
     ...
     &          b,c
        ...
        ut(1,1) = dreal(squlmx(3,3)) ! ut(1,l)
        ut(1,2) = dreal(squrmx(3,3)) ! ut(1,r)
        ut(2,1) = dreal(squlmx(6,3)) ! ut(2,l)
        ut(2,2) = dreal(squrmx(6,3)) ! ut(2,r)
        ub(1,1) = dreal(sqdlmx(3,3)) ! ub(1,l)
        ub(1,2) = dreal(sqdrmx(3,3)) ! ub(1,r)
        ub(2,1) = dreal(sqdlmx(6,3)) ! ub(2,l)
        ub(2,2) = dreal(sqdrmx(6,3)) ! ub(2,r)
...
c-------------------- carena-espinosa-quiros-wagner, plb 355 (1995) 209
        ...
        if (mass(ksqu_flav(3,1)).eq.0.0d0)  call dssfesct(unphys)
        if (unphys.ne.0) return
     ...
     &       log(mt2/msqz))
        ...
        ms2 = 0.5d0*(mass(ksqu_flav(3,1))**2 + mass(ksqu_flav(3,2))**2) + mt2
        ms4 = ms2**2
        t = log(ms2/mt2)
...
c---------------------------------------------- unrecognized higloop
        ...
        write(dsi2s,'(i8)') higloop
        call dswrite(0,1,'unrecognized option higloop='//dsi2s)
        stop
      endif
