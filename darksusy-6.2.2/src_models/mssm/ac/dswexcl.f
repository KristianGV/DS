...
c=======================================================================
      ...
      if (btest(excl,5)) write (unit,1000) 'Higgs mass (mass is excluded); '
      if (btest(excl,6)) write (unit,1000) 'neutralino mass; '
      if (btest(excl,7)) write (unit,1000) 'Br(b->s+gamma); '
      if (btest(excl,8)) write (unit,1000) 'delta rho; '
      if (btest(excl,9)) write (unit,1000) '(g-2)_mu; '
      if (btest(excl,10)) write (unit,1000) 'Br(B_s->mu+ mu-); '
      if (btest(excl,11)) write (unit,1000) 'Comb. squark-gluino; '
      if (btest(excl,12)) write (unit,1000)
     &   'HiggsSignals returned p-value < 1%; '
