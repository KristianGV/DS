*****************************************************************************
*** function phiith integrates dssedydth over the angle theta.
...
c------------------------ variables ------------------------------------
      ...
      real*8 mp1t,mp2t
      integer chi,istat,kind,type,seerror
      character*2 wh

      real*8 sum,emax,emin,e1cm
      logical wb,chok

c------------------------ functions ------------------------------------

      external dssedydth
      real*8 dssedydth,dsseyield_int
...
c...Set mass for virtual particles
      if (e00.lt.m00) then
         m00=e00*0.9999d0
      endif
...
c        else
c          write(*,*) 'error in dsseyieldfth: a particle with mass ',m0
...
c          write(6,*)
c          write(6,5000)
c     +     'DS WARNING in dsseyieldfth: WIMP masses of less than ',
...
c          write(6,*)
c          write(6,5000)
c     &      'DS WARNING in dsseyieldfth: a neutralino mass down to ',
...
c          endif
      ...
      if (emax.gt.ub(chi)) then
        if (prtlevel.gt.0) then
          write(6,*)
          write(6,5000) 
     &      'DS WARNING in dsseyieldfth: a neutralino mass up to ',
     ...
     +      ' lower bound.'
      ...
      if (wb) then
        if (wh.eq.'su'.or.wh.eq.'SU') then
          sum=dsseyield_int(dssedydth,cos(thu),1.0d0,chi,1,m00,mp1t,mp2t,
     &    e00,emuthr,thmax,kind,type,seerror)
        else
          sum=dsseyield_int(dssedydth,cos(thu),1.0d0,chi,2,m00,mp1t,mp2t,
     &    e00,emuthr,thmax,kind,type,seerror)
        endif

        dsseyieldfth=sum*1.0d-15
