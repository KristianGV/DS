...
*****************************************************************************
      ...
      real*8 function dsIByieldone_fsr(mwimp,emuthr,FSRch,yieldk,istat)
      implicit none
      include 'dsmssm.h'
      include 'dsibcom.h'
...
c------------------------ functions ------------------------------------
      ...
      real*8 dsIBfsrdxdy,dsIBf_intdy,dsIBf_intdxdy
      external dsIBfsrdxdy
...
c...only if kinematically allowed go on to compute photon flux
      if (x.le.(0.999*(1-mf**2/mwimp**2))) then
          ...
          if (yieldk.eq.52) then        ! integrated photon flux above x
            tmpresult=dsIBf_intdxdy(FSRch+100,x,mwimp,mf,mf)   
          elseif (yieldk.eq.152) then   ! differential photon flux at x
            tmpresult=dsIBf_intdy(FSRch+100,x,mwimp,mf,mf)/mwimp
          endif

      endif

      dsIByieldone_fsr=tmpresult
      istat=iberr
