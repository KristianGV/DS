      program dstest_silveira_zee
c
c     This program tests the Silvereira_Zee module, and many general DarkSUSY routines
c
c     In addition to the DarkSUSY programs and data files, this test
c     program needs the input file dstest_silveira_zee.mod provided with the
...
c Set up real testing of DS, by comparing to pre-computed values
      ...
      data SAVSHORT/'DStest_Silveira_Zee_sav.dat'/       ! default : 'DStest_Silveira_Zee_sav.dat'
      data testunit/98/
...
c
      ...
      if (moduletag.ne.'silveira_zee') then
         write(*,*)
     &     'Sorry, dstest_silveria_zee is only designed for the Silveira_Zee module and'
        write(*,*) 'you compiled it with module ',moduletag
        write(*,*)
        write(*,*) 'Set DS_MODULE=silveira_zee in /examples/test/makefile' //
     &              ' and try again...'
        stop
      endif
      
      write (*,*) 'Now starting dstest_silveira_zee... '
      savfile=savshort
...
*******************************************************
***       read in + set up Silveira-Zee models      ***
*******************************************************
      modfile = 'models/dstest_silveira_zee.mod'
...
c... start loop to read in models      
 1000 call read_model_silveira_zee(11,0,iend,ierr)
      ...
      if (ierr.ne.0) then
         ...
         write (*,*)
         call dswrite(0,0,'Model parameters read from Silveira_Zee model file.')
      endif
...
c No Spin-dependent scattering in Silveira-Zee !
...
********************************************************************
      ...
      subroutine read_model_silveira_zee(lunit,nmodel,iend,ierr)
c
 ...
 3000 continue
      ...
      subroutine testvalue(testunit,testlevel,msg,value)
        ...
        if (msg.eq.'final call') then
          write(*,*) 'Total number of errors in dstest_silveira_zee: ',memory
          write(*,*)
          write(*,*)
c The simpler stop + integer only works for FORTRAN >= 2008...   
c          stop memory
          open(unit=99, file="dstest_silveira_zee.out", status="unknown")
          write(99,'(i3)') memory
          close(99)
          stop
c          return
