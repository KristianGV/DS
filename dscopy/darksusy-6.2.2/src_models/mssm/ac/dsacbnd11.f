...
c...First, we have a call to FeynHiggs. This implementation does not seem
c...very good though, and we will not use it. Instead, we use SuperIso
c...called via a SLHA file. The obsolete FeynHiggs call is kept
c...for reference.
c
...
c      endif
     ...
     &  brbtaunu_mssm,brbdtaunu_mssm,amu_mssm,rmu23_mssm,error)
      bsgsi=bsg_MSSM ! SuperIso b->s gamma calculation
      delta0=delta0_MSSM
      bsmumu=bsmm_mssm
...
c     &    .or.
c     &    (mass(ksqd(i)).lt.224.d0 .and. mass(ksqd(i)).gt.mass(kgluin)) !corrected u->d 
c      enddo                                                             !ms 021001
...
c...using HiggsBounds
         ...
         if (HBresult.ne.1) Excl=ibset(Excl,5)
...
c...as our limit
      ...
      if (gm2mu.lt.-2.7d-10.or.gm2mu.gt.49.1d-10) then
         excl=ibset(excl,9)
      endif
