...
c10    format (1x,a2,' -> dsff(',i2,')  partial=',e8.2)
c11    format (1x,a2,' -> ww      partial=',e8.2)
c12    format (1x,a2,' -> zz      partial=',e8.2)
c13    format (1x,a2,' -> n',i1,'n',i1,'    partial=',e8.2)
...
c        write(*,*) 'qcd, noqcd = ',hwidthqcd,hwidthnoqcd
     ...
     &    hwidthqcd
      ...
      if(fact.gt.0.d0.and.higwid.gt.0) then
...
c        write(*,*) 'qcd width = ',hwidthqcd
      else
        write(*,*) 'troubles in dshigwid for model ',idtag
        write(*,*) 'Higgs particle = ',pname(khiggs)
        write(*,*) 'Gluons in final state'
        write(*,*) 'qcd width = ',hwidthqcd
      endif  
...
c        write(*,*) 'Gluons in final state'
c        write(*,*) 'next to leading qcd term = ',hwidthqcdnl
      else
        write(*,*) 'troubles in dshigwid for model ',idtag
        write(*,*) 'Higgs particle = ',pname(khiggs)
        write(*,*) 'Gluons in final state'
        write(*,*) 'next to leading qcd term = ',hwidthqcdnl
      endif
c
c
...
c----------------------------------------------------------- into quarks
     ...
     &        4*mi*mj*dreal(gl(khiggs,k2,k1)*gr(khiggs,k2,k1)))
          width(khiggs)=width(khiggs)+partial
          vvv((i-1)*3+j)=partial
...
c        write(*,*) 'qcd, noqcd = ',hwidthqcd,hwidthnoqcd
     ...
     &    hwidthqcd
      ...
      if(factr.gt.0.d0.and.factl.gt.0.d0.and.higwid.gt.0) then
c... redefine the couplings
...
c---------------------------------------------------------- into leptons
     ...
     &      ((mh**2-mi**2-mj**2)*
     &      (dsabsq(gr(khiggs,k2,k1))+dsabsq(gl(khiggs,k2,k1)))-
     &      4.0d0*mi*mj*dreal(gl(khiggs,k2,k1)*gr(khiggs,k2,k1)))
          width(khiggs)=width(khiggs)+partial
          vvv(i)=partial
c          write(*,*) pname(k1),pname(k2),partial
        endif
