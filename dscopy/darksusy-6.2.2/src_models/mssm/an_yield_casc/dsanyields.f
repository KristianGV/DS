...
c------------------------ functions ------------------------------------
      ...
      real*8 dsanyields2,dsanemean,dsanyieldfth,dsanyielddec
...
c      write(*,*) 'dsanyields called with:'
c      write(*,*) '     eh = ',eh
c      write(*,*) '  egev = ',egev
c      write(*,*) '    hno = ',hno
c      write(*,*) '   yieldk = ',yieldk
...
c..."complex" channels
     ...
     &      3,yieldk,istat)
        ...
        if (ans0br(7,hno).gt.0.0d0) then    ! S+ S- channel
          ...
          yield=yield+2.0*
     &      ans0br(7,hno)*dsanyields2(e1,egev,4,yieldk,istat)
        endif
     ...
     &       ans0m(1),egev,9,yieldk,istat)
          yield=yield+ans0br(8,hno)*dsanyields2(e2,egev,1,yieldk,istat)
        endif
     ...
     &       ans0m(2),egev,9,yieldk,istat)
          yield=yield+ans0br(9,hno)*dsanyields2(e2,egev,2,yieldk,istat)
        endif
     ...
     &       ans0m(3),egev,9,yieldk,istat)
          yield=yield
     &      +ans0br(10,hno)*dsanyields2(e2,egev,3,yieldk,istat)
        endif
     ...
     +    dsanyieldfth(eh,ans0m(hno),msim(8),anscm,egev,8,yieldk,istat)
          yield=yield
     &        +ans0br(11,hno)*dsanyields2(e2,egev,4,yieldk,istat)
        endif
