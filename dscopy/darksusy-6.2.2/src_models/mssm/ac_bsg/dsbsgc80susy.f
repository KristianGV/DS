...
c   from DarkSUSY
      ...
      if (mass(ksqu_flav(3,1)).ge.mass(ksqu_flav(3,2))) then   ! DarkSUSY convention matches
        do i=1,2
         st1(i)=mass(ksqu_flav(3,1))**2/mass(kcha(i))**2
         st2(i)=mass(ksqu_flav(3,2))**2/mass(kcha(i))**2
         sq(i)=mcomsq**2/mass(kcha(i))**2
        enddo
        mst1=mass(ksqu_flav(3,1))
        mst2=mass(ksqu_flav(3,2))
        thetast=atan(-dble(conjg(squlmx(6,3)))
     &             /dble(conjg(squrmx(6,3))))

      else        ! DarkSUSY convention differs. Switch 1 <-> 2
        do i=1,2
         st1(i)=mass(ksqu_flav(3,2))**2/mass(kcha(i))**2
         st2(i)=mass(ksqu_flav(3,1))**2/mass(kcha(i))**2
         sq(i)=mcomsq**2/mass(kcha(i))**2
        enddo
        mst1=mass(ksqu_flav(3,2))
        mst2=mass(ksqu_flav(3,1))
        thetast=atan(-dble(conjg(squlmx(6,3)))
