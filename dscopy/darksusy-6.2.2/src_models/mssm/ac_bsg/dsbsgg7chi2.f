...
c     \Delta's, but different input parameters 
      ...
      if (mass(ksqd_flav(2,1)).gt.mass(ksqd_flav(2,2))) then
        x1s=mass(ksqd_flav(2,1))**2/mass(kgluin)**2
      else
        x1s=mass(ksqd_flav(2,2))**2/mass(kgluin)**2
      endif

      if (mass(ksqd_flav(3,2)).lt.mass(ksqd_flav(3,1))) then
        x2b=mass(ksqd_flav(3,2))**2/mass(kgluin)**2
      else
        x2b=mass(ksqd_flav(3,1))**2/mass(kgluin)**2
      endif
