***********************************************************************
*** dskdm2 returns the full scattering amplitude for NEUTRALINO DM at zero 
*** momentum transfer squared, SUMMED over both initial and final spin and 
...
***          2018-05-14 (moved momentum->energy conversion to src_models/)
***********************************************************************
      ...
      real*8  omega,omegax,xf,tmpres
      integer SMtype,kf,ksf(2),nsf,i,j

c... propagators and various coupling combinations
      complex*16 ds(2),du(2),dtZ,dtH(2)
      complex*16 omgllrr(2,2),mfglrrl(2,2)
...
c... is loaded, and should (at least) be included for all interface functions
      call dscheckmodule('MSSM','dskdm2')
...
c... setup (s)particles
      ...
      if (SMtype.eq.10) kf=kc
      if (SMtype.eq.11) kf=kb
      if (SMtype.eq.12) kf=kt
      if (SMtype.ge.10) return
      
      omega = sqrt(mass(kf)**2+omega**2) ! convert from momentum (input) 
                                         ! to energy (output)
      
      nsf=2
      if (SMtype.le.3) nsf=1
        do 10 i=1,nsf
          if (SMtype.le.3) ksf(i)=ksnu_flav(SMtype,1)
          if (SMtype.ge.4.and.SMtype.le.6) ksf(i)=ksl_flav(SMtype-3,i)     
          if (SMtype.eq.7) ksf(i)=ksqu_flav(1,i)
          if (SMtype.eq.8) ksf(i)=ksqd_flav(1,i)
          if (SMtype.eq.9) ksf(i)=ksqd_flav(2,i)
          if (SMtype.eq.10) ksf(i)=ksqu_flav(2,i)
          if (SMtype.eq.11) ksf(i)=ksqd_flav(3,i)
          if (SMtype.eq.12) ksf(i)=ksqu_flav(3,i)
  10    continue
...
c... sum all diagrams and correct for dof
        ...
        tmpres=2.*tmpres*nsf
        if (SMtype.ge.7) tmpres=tmpres*3. ! color factor for quarks


      if (tmpres.lt.0d0) then
        if (memory.ne.idtag) then
          write(*,*) 'WARNING: negative |M|^2 in dskdm2.f for model',
     &               idtag, ' !'
