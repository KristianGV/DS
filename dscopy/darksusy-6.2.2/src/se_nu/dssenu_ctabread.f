...
c...Make sure 'num' and 'numc' is treated consistenly     
      ...
      if (prtlevel.ge.2) write(*,*) 'dssenu_ctabread: Opening file ',file
      open(unit=13,file=file,
     &  form='formatted',status='old',err=200)


      read(13,101) whfile,vdf,ncfile
...
c...Check consistency of files
      ...
      if (wh.eq.'su'.or.wh.eq.'SU') then
        ...
        if (nc.ne.ncfile) bad=.true.
        if (bad) then
          write(*,*) 'ERROR in dssenu_ctabread: ',
     &      'File type mismatch.'
