...
c...specifically the more general expressions in appendix A.
      ...
      real*8 r,ni,e0,sla1,sla2,sla3,v,dssem_sunvesc,vp,
     &  dssem_sundenscomp,foveru
      integer vtype
      external foveru
      common/seint/mx,ma,sigsi,sigsd,r,vtype
      if (sctype.eq.1) then ! SI
        siga=sigsi*sdaa(zx,ix)**2*(mx*ma/(mx+ma))**2
     &     /(mx*m_p/(mx+m_p))**2
      else ! SD
        siga=0.d0
      endif
...
c...lower escape velocity after to make sure it does not reach Jupiter
      ...
      if (zx.eq.1.and.ix.eq.1) then ! no form-factor suppression for Hydrogen

        if (sctype.eq.2) then ! SD
...
c...to choose only scatters that reach out to Jupiter
     ...
     &    -muminus**2/mu*((u/c_light)**2+(veout/c_light)**2)) ! modified gould (2.13)
        dssenu_csint2=foveru(u)*sla3 ! gould (2.13) + (2.8)
...
c...Gould expressions).
        ...
        dssenu_csint2=foveru(u)*sla3*(sla1-sla2)  ! gould (A6) + (2.8)

      endif


c...we have two factors of c missing, add them to get units cm^-4
      dssenu_csint2=dssenu_csint2*(3.0d10)**2
c...dssenu_csint2 is now equal to f(u)/u * w Omega_v^-(w), i.e.
c...f(u)/u * (A6) in Gould.
