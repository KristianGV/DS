...
c--------------------------------------------------------------------------
       implicit none
       real*8 mx,v_star,v_bar,dsmoderf,apil
       real*8 eta,etahat,ahatpl,ahatmin
       real*8 etapil,apilpl,apilmin,a0,b0,m_a,betaplus,q_a,
     1      r_a2,betaminus,
     2      aa,ahat,vesc,phi
       real*8 sla1
       include 'dshmcom.h'
...
c...q_a = 2*Q0 in jkg (9.12)
       q_a=3./2./m_a/r_a2
       a0=mx*(v_bar/c_light)**2/(3.0d0*q_a)    ! gould (A7)
...
c
       ...
       sla1=exp(-a0*etahat**2)/sqrt(1.+a0)*(2.*dsmoderf(etahat)+
     1      dsmoderf(ahatmin)-dsmoderf(ahatpl))-
     2      exp(-b0*etapil**2)/sqrt(1+b0)
     3        *exp(-(a0-b0)*aa**2)*
     4       (2.*dsmoderf(etapil)+
     5      dsmoderf(apilmin)-dsmoderf(apilpl))
       sla1=sla1/2./eta/b0*sqrt(8./3./pi)
       dssenu_sefull=sla1  ! gould (A10), apart from sigma n n_w v_bar
c       write(*,*) 'dssenu_sefull: ',mx/m_a,v_star,v_bar,dssenu_sefull
        return
        end
