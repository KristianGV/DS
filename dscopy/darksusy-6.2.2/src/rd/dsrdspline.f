...
c     yy2(1)=0.0d0 if zero 2nd derivative
      u(1)=0.0d0
      istart=1
      iend=1 ! not really needed, but gets rid of warnings that iend might be uninitialized
...
c...PS 20110424; split evaluation of conditional to prevent indexing pp(0)
        if (i+1.eq.nr) then
           iend=i+1
          goto 30
        endif
