...
c **********************************************************************
      ...
      subroutine dgadap2_thav(a0,b0,f,func2,eps0,sum)
******************************************************************
******************************************************************
...
c  made real*8 by j. edsjo 97-01-17
    ...
    1 format(16h gadap:i too big)
      ...
      goto 11 ! Addition: skip printout of added points 
 10   continue                  ! Addition
      if (prtlevel.ge.3) then
         write(*,*) 'dgadap2_thav: ',nrd-nrdentry,' points added. ',
     &        nrd,' points in total.'
      endif
 11   continue
      nrdentry=nrd ! Addition
      eps=eps0
      ...
      a(1)=a0
      b(1)=b0
      f1(1)=f(func2,0.5*(1+c)*a0+0.5*(1-c)*b0)
      f2(1)=f(func2,0.5*(a0+b0))
      f3(1)=f(func2,0.5*(1-c)*a0+0.5*(1+c)*b0)
      ifu=3
      if (nrd.ne.nrdentry) goto 10 ! Addition: restarts, points added
  ...
  100 continue
      ...
      w1=a(i)+(b(i)-a(i))/5.
      u2=2.*w1-(a(i)+a(i+2))/2.
      f1(i+1)=f(func2,a(i)+b(i)-w1)
      f2(i+1)=f3(i)
      f3(i+1)=f(func2,b(i)-a(i+2)+w1)
      f1(i+2)=f(func2,u2)
      f2(i+2)=f2(i)
      f3(i+2)=f(func2,b(i+2)+a(i+2)-u2)
      f1(i+3)=f(func2,a(i)+a(i+2)-w1)
      f2(i+3)=f1(i)
      f3(i+3)=f(func2,w1)
      ifu=ifu+6
      if (nrd.ne.nrdentry) goto 10 ! Addition: restart, points added
  ...
  110 continue
      ...
      l=l-1
      goto 110
  120 write(*,*) 'warning in dgadap2_thav: too high i (>300)'
      write(*,*) '  for model ',idtag
      goto 130
  125 write(*,*) 'warning in dgadap2_thav: too many function calls (>5000)'
      write(*,*) '  for model ',idtag
  130 return
      end
