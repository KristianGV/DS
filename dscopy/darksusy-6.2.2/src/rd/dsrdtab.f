...
c...Determine minimum relative delta p at threholds to add points
           if (fast.le.1) then
              dpr=1.d-9
           else ! more restrictive on which points to add for fast>1
              dpr=dpminr
 ...
 17     if (pth(i).lt.pthlow) pthlow=pth(i)
        if (pth(i).lt.pmax) then
          call dsrdaddpt(wrate,pth(i)-dpth,deltap,dpr) ! je test
          if (rderr.ne.0) goto 999 ! return 
          w1=exp(yy(nr))
          do j=nthup,1,-1
            ...
            call dsrdaddpt(wrate,pth(i)+dble(j)**2*dpth,deltap,dpr)
            if (rderr.ne.0) goto 999 ! return 
            w2=exp(yy(nr))
...
c------------------------------------- add points below lowest threshold
      if (nth.ge.1) then
        do i=1,3
          call dsrdaddpt(wrate,pthlow*dble(i)/4.0d0,deltap,dpr)
          if (rderr.ne.0) goto 999 ! return 
        enddo
      endif
...
cc          write(*,*) 'now adding ',nadd,' points to nr = ',nr
          ...
          enddo
          do i=1,nadd
            call dsrdaddpt(wrate,padd(i),deltap,dpr)
            if (rderr.ne.0) goto 999 ! return 
          enddo
          do i=1,nr
            pp(i)=pp(i)*deltap
...
c...note that in the j loop, we go from nth (and not nth-1)
c...as pth(nth+1) is set above
      if (nth.gt.1) then
        do i=0,nth
          do j=nth,i,-1
cc            write (*,*) 'i,j',i,j
