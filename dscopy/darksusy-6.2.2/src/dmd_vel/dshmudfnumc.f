...
c        write(*,*) vmax,umax
     ...
     &                       /(2.d0*uint*v_obs))
          ...
          xudfnum(ii)=uint
          yudfnum(ii)=0.5d0*result*4.d0*pi*uint ! km^-2 s^2
...
c          write(*,*) ii,xudfnum(ii),yudfnum(ii)
 ...
 30     continue
     ...
     &                       /(2.d0*uint*v_obs))
          ...
          enddo
          xudfnum(ii+1)=uint
          yudfnum(ii+1)=0.5d0*result*4.d0*pi*uint ! km^-2 s^2
c          write(*,*) ii,xudfnum(ii+1),yudfnum(ii+1)
 ...
 102    format('#','u [km/s]       u*DF [(km/s)^-2]')
        ...
        enddo
        close(dfunit)
        call dsspline(xudfnum,yudfnum,int(reudfnum),1.d31,1.d31
     &            ,yudfnum2)
        udfload=.false.  ! do not load next time
      endif
      if(u.ge.xudfnum(1).and.u.le.xudfnum(int(reudfnum))) then
        call dssplint(xudfnum,yudfnum,yudfnum2,int(reudfnum),u,res)
        dshmuDFnumc=res
        return
      else
        dshmuDFnumc=0.d0
