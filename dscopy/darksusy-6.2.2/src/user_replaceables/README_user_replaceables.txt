README for user_replaceables in src/
===================================
...
in src/. Replacing routines in src_models/ is handled in the same way
in the respective user_replaceables directories in those modules.
...
4. DarkSUSY will now have created proper makefiles that put your replaced
   routines in a library libds_main_user.a and made sure that the main programs
   are created by linking to these files first (see makefile in
   examples/test).
