       ...
       e_nu=x
       fluxnu=dssea_honda(1,e_nu,c_th)
       fluxnubar=dssea_honda(2,e_nu,c_th)
       r=fluxnu/fluxnubar
...
c       epsilon=522. ! rock values, je phd thesis
       ...
       if (fltype.eq.1) then ! impinging fluxes
         dssea_ff=avogadro*sigma/alpha*(fluxnu+fluxnubar)
c       dssea_ff=dssea_ff/(1.+e_mu/epsilon)*
c     1       (e_nu-e_mu+a/3.*(e_nu-e_mu*(e_mu/e_nu)**2)/e_nu**2)
         dssea_ff=dssea_ff/(1.+e_mu/epsilon)*
     1       (e_nu-e_mu+a/3.*(e_nu-e_mu*(e_mu/e_nu)**2)) !je corr 980603
         dssea_ff=dssea_ff*1.d-12 ! 23 from avo; -38 from sigma; 3 from alpha
       else ! contained events
         dssea_ff=avogadro*sigma*(fluxnu+fluxnubar)
         dssea_ff=dssea_ff*(1.0+a*(e_mu/e_nu)**2)
         dssea_ff=dssea_ff*1.d-15  ! 23 from avo; -38 from sigma
       endif
       return
       end
