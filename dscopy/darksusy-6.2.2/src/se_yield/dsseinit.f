*****************************************************************************
*** subroutine dsseinit initializes and loads (from disk) the common
...
c------------------------ variables ------------------------------------
      ...
      integer i,j,k,l,m,flxkfile,fl,kind,type,yli
...
c...Set-up
      if (selast(kind2ki(kind)).lt.semax) then ! still space in memeory
         selast(kind2ki(kind))=selast(kind2ki(kind))+1
         yload(kind2ki(kind),type)=selast(kind2ki(kind))
         yli=yload(kind2ki(kind),type) ! load index
         if (prtlevel.ge.2) write(*,'(A,I2,A,I3)') 
     &      'Loading WIMP annihilation yield for kind = ',
     &      kind2ki(kind),' and type = ',type
      else
         write(*,*)
     &    'ERROR in dsseinit. You have requested to load more WIMP'
         write(*,*)
     &    'annihilation yields than DarkSUSY is configured to handle.'
         write(*,*) 'Maximal number of simultaneous yields = ',semax
         write(*,*) 
     &     'To come around this,',
     &     ' increase semax in include/dsseyieldcom.h'
         write(*,*)
...
c...integrated yields (lower end of each bin)
      do i=-1,zn2
        zindex(i,1)=dble(i)*1.d0/dble(zn2)
      enddo

c...differential yields (center of each bin)
      do i=-1,zn2
         zindex(i,2)=(dble(i)+0.5d0)*1.d0/dble(zn2)
         dz(i)=1.0d0/dble(zn2)
      enddo
...
c...generate file name
      if (seftype.eq.'a'.or.seftype.eq.'a') then
        write(*,*) 'DS ERROR in dsseinit:'
        write(*,*) 
     ...
     &    ' use binary instead.'
      ...
      if (kind2ki(kind).eq.1) then   ! integrated
        filepref=sebase//'-fl21-f'
        write(filenr,'(i2)') type
      else                    ! differential
        filepref=sebase//'-fl22-f'
        write(filenr,'(i2)') type
      endif
...
c...Create mixed tables, differential in energy, integrated in theta
                do l=0,zn2-1
                  tmpy=0.d0
                  do m=0,thn
                     tmpy=tmpy+dble(phidiff(m,l,k,j,i,yli))*real(dth(m))
                     phimixed(m,l,k,j,i,yli)=real(tmpy)
                  enddo
                enddo
...
c                  phidiff(-1,l,k,j,i,flvar)=phidiff(0,l,k,j,i,flvar)
     ...
     &              4.0/3.0*phidiff(0,l,k,j,i,yli)   ! je 961023
                ...
                do m=-1,thn
                  ...
                  phidiff(m,zn2,k,j,i,yli)=phidiff(m,zn2-1,k,j,i,yli)
                  phimixed(m,-1,k,j,i,yli)=phimixed(m,0,k,j,i,yli)
                  phimixed(m,zn2,k,j,i,yli)=phimixed(m,zn2-1,k,j,i,yli)
                enddo
              endif
            enddo
          enddo
...
c     &    'loading of WIMP annihilation yield tables finished.'
 ...
 100  continue
      write(*,*) 'ERROR in seinit: while opening file ',filein
      ...
      block data dsseyieldcom
      implicit none
      include 'dsseyieldcom.h'

c...Masses for internal channel numbers chi 1-14
      data msim/0.010d0,0.005d0,0.2d0,1.5d0,4.8d0,175.d0,
...
c...Lowest mass index for the 14 channels chi
      ...
      data seerr/0/
      data seftype/'b'/
c      data sebase/'we-res-b-full'/
c      data sebase/'we-res-may2012'/
c      data sebase/'we-res-jan2013'/
      data sebase/'we-res-apr2017'/
