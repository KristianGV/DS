...
c
c  This program is a special version of flxconv that is just made to
c  scan over several masses and calculate the conversion factors (and
c  write them to a file). For actual interactive calculations, use flxconv
c  instead.
c
...
c...Default local halo density      
      ...
      wh='su'
      ch=8
      write(*,*) 'Channel: ',chname(ch)
      denscorr=0.92d0

c...Starting fluxes
      type1=27
      write(*,*) 'Starting flux type: ',typename(type1)
      kind1=1 ! 1=integrated, 2=differential
      e1=1.d0 ! energy (threshold)
      th1=30.d0 ! half-angle of cone

c...Ending fluxes
      type2=31
      write(*,*) 'Ending flux type:   ',typename(type2)
      kind2=0 ! 1=integrated, 2=differential
      e2=0.d0 ! energy (threshold)
      th2=0.d0 ! half-angle of cone

      do ch=1,15
         write(*,*) 'Channel: ',ch,chname(ch)
c...Output data file
      if (ch.lt.10) then
         write(ch1,'(I1)') ch
         filename='flxconvplot-'//wh//'-'//ch1//'.dat'
      else
         write(ch2,'(I2)') ch
         filename='flxconvplot-'//wh//'-'//ch2//'.dat'
      endif
      open(unit=47,file=filename,status='unknown',
     &   form='formatted')
      write(47,'(A,I2,1x,A)') '# Channel: ',ch,chname(ch)
      write(47,'(A,A)') '# Where: ',wh
      write(47,'(A,1x,I2,1x,I1,1x,E14.6,1x,E14.6)') 
     &  '# Starting: ',type1,kind1,e1,th1
      write(47,'(A,1x,I2,1x,I1,1x,E14.6,1x,E14.6)') 
     &  '# Ending:   ',type2,kind2,e2,th2
      write(47,'(A,A)') '#   i           mass           cf2a',
     &   '           ca2f           cf2f'

      nm=100 ! number of mass points
      do ni=0,nm
         mneu=10**(dble(ni)/dble(nm)*3.d0+1.d0)
         write(*,*) 'Mass: ',ni,mneu       
...
c...Now write results
         write(47,101) ni,mneu,cf2a,ca2f,cf2f
 101     format (1x,I4,4(1x,E14.6))

      enddo ! end mass lloop
      close(47)
      enddo ! end ch loop
 ...
 113  format(A,F10.2,A,A2,A,I2,A,F8.4,A)
     ...
     &   'Unit: ',units(typedim(type2))
      write(*,*) ' '
      write(*,*) 'For last mass point we have: '
